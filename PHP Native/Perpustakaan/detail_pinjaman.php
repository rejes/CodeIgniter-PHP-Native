...
$peminjaman_detail = mysqli_query($conn, "SELECT peminjaman_detail.id, peminjaman_detail.denda_terlambat, peminjaman_detail.denda_lainnya, koleksi.judul, koleksi_stok.kode FROM peminjaman_detail JOIN koleksi_stok ON koleksi_stok.kode = peminjaman_detail.koleksi_stok_kode JOIN koleksi ON koleksi.id = koleksi_stok.koleksi_id JOIN penerbit ON penerbit.id = koleksi.penerbit_id WHERE peminjaman_detail.peminjaman_id = '$id' GROUP BY peminjaman_detail.id");
