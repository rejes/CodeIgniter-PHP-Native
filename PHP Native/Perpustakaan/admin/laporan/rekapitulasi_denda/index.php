...
$peminjaman = mysqli_query($conn, "SELECT koleksi.judul, jenis.nama, SUM(peminjaman_detail.denda_terlambat + peminjaman_detail.denda_lainnya) as total_denda FROM peminjaman_detail LEFT JOIN peminjaman ON peminjaman.id = peminjaman_detail.peminjaman_id LEFT JOIN koleksi_stok ON koleksi_stok.kode = peminjaman_detail.koleksi_stok_kode LEFT JOIN koleksi ON koleksi.id = koleksi_stok.koleksi_id LEFT JOIN jenis ON jenis.id = koleksi.jenis_id WHERE koleksi.judul IS NOT NULL $sql_clause GROUP BY koleksi.id ORDER BY peminjaman.tanggal_pinjam ASC");
