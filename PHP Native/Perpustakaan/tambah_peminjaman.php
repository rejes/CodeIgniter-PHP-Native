      ...
      <div class="card-body">
        ...
        <form action="" class="row" method="POST">
          <div class="col-6 mb-3">
            ...
            <input type="text" class="form-control" value="<?= $_SESSION['fo_nama']; ?>" disabled>
          </div>
          <div class="col-6 mb-3">
            ...
            <table id="datatable" class="table table-bordered table-striped">
              ...
              <tbody>
                <?php while($row = mysqli_fetch_assoc($koleksi)) { ?>
                  <?php 
                    $koleksi_id   = $row['id'];
                    $koleksi_stok = mysqli_query($conn, "SELECT koleksi_stok.kode, COUNT(peminjaman_detail.id) sa FROM koleksi_stok LEFT JOIN peminjaman_detail ON koleksi_stok.kode = peminjaman_detail.koleksi_stok_kode WHERE (koleksi_stok.koleksi_id = '$koleksi_id' OR peminjaman_detail.status IN ('tolak', 'lengkap')) AND ((SELECT COUNT(*) FROM peminjaman_detail q WHERE q.koleksi_stok_kode = koleksi_stok.kode) < 1) LIMIT 1");
                  ...
                  <?php if(mysqli_num_rows($koleksi_stok) > 0) { ?>
                    ...
                    <tr class="text-center">
                      <td class="align-middle">
                        <input type="checkbox" name="koleksi_stok_kode[]" value="<?= $data_stok['kode']; ?>" <?= $checked; ?>>
                      </td>
                      <td class="align-middle"><?= $row['judul']; ?></td>
                      <td class="align-middle"><?= $row['nama']; ?></td>
                      <td class="align-middle"><?= $row['total_stok']; ?></td>
                    </tr>
                  <?php } ?>
                <?php } ?>
              </tbody>
